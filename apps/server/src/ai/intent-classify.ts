...
/**
 * 中文 / 英文常见疑问词与请求语；用于无 LLM 时的启发式判定。
 * 偏向：宁可错判成 question（让 QA 兜底回答 "无法从材料确认"），
 * 也不要把用户的真实问题静默判成 irrelevant 丢掉（最差体验）。
 */
const QUESTION_HINTS_ZH = [
  // 5W1H / 疑问词
  '什么', '为什么', '为啥', '怎么', '怎样', '咋', '如何',
  '何时', '何地', '何处', '何人',
  // 单字 "哪"：覆盖 哪个/哪里/哪些/哪儿/哪种/哪一/哪天/哪类 …
  '哪',
  // 是非 / 选择疑问
  '是不是', '能不能', '可不可以', '行不行', '有没有', '是否', '能否', '可否',
  // 句末助词性问句
  '吗', '呢', '啥',
  // 礼貌 / 间接问法（语义=问）
  '请问', '想问', '问一下', '问问', '问下', '请教', '想知道', '想了解', '了解一下',
  // 描述性请求（语义=请讲解/请扩展，等价于question走QA）
  '介绍一下', '介绍下', '说一下', '说下', '说说', '讲讲', '讲一下',
  '讲解', '解说', '展开', '详细', '解释',
  // 对比 / 原理 / 含义
  '区别', '不同', '差异', '差别', '原理', '含义', '意思',
  '是什么', '叫什么', '指什么', '代表什么', '怎么理解', '如何理解',
  // 数量 / 对象
  '多少', '几个', '几条', '几种', '几类', '几位', '谁',
];
...
判断准则：
- 「问题」：用户希望你解释、扩展、回答与 PPT 主题相关的问题；典型信号包括疑问词（什么/为什么/怎么/如何/哪/哪些/多少/谁/啥/吗/呢）、问号、或上下文里清楚是在问；
  也包括 "我想问 / 请问 / 问一下 / 想知道 / 介绍一下 / 讲一下 / 解释一下" 等请求性问法；
  ASR 可能截断或转错（错别字、口误），只要能合理推断用户在问 PPT 主题（包含其行业/政策/产品/案例 等延伸领域）就视为"问题"。
- 「无关」：闲聊、与 PPT 主题完全无关、纯噪音/口误转写、或仅仅是在念读 PPT 文字。
- 拿不准时优先判 "question"（让下游 QA 层基于材料决定是否能作答），不要因为转写信息不全就当噪音丢弃。
